...
**学习内容**：P71-P76类型注解+模块（函数类型注解、导入、自定义、包）

**今日代码**：无新增代码（理论学习）
- 学习笔记[nodes.md](./day5/nodes.md)

**遇到问题**：
- 通过 from 包名 import * 导入包下所有模块时报错

**解决方案**：
- 导入包下需要的所有模块需要在__init__.py文件中添加__all__=["模块名","模块名"]
- 导入模块需要的所有功能需要在定义模块的文件中添加__all__=["功能名","功能名"]

**额外收获**：
- 类型注解也是排查报错的一种方式，对于函数返回值的类型注解一般要用[]
- 导入模块中功能可以从 
1）相对路径：从当前文件所在目录开始查找 eg:from 包名.模块名 import 功能名。
2）绝对路径：从项目的根目录下开始查找 eg:from 根目录.包名.模块名 import 功能名
- __all__可以防止命名被占用

**明日计划**：P77-P85面向对象基础（类与对象、实例方法、魔法方法、实例属性、教务系统案例）
